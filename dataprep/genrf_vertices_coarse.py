...
def create_mesh(lx: float, ly: float, nx: int, ny: int) -> Tuple[np.ndarray, np.ndarray, np.ndarray]:
    """
...
def compute_covariance_matrix(coords: np.ndarray, correlation_length: float, 
...
def compute_kl_decomposition(cov_matrix: np.ndarray, 
                           error_threshold: float = 1e-3) -> Tuple[np.ndarray, np.ndarray, int]:
    """
...
def plot_eigenvalues(eig_vals: np.ndarray, error_func: np.ndarray, 
                    output_path: Path, n_points: int) -> None:
    """
...
def generate_random_fields(n_samples: int, eig_vals: np.ndarray, eig_vecs: np.ndarray, 
                           mu_target: float, sigma_target: float) -> Tuple[np.ndarray, np.ndarray]:
    """
...
def save_samples(gaussian_samples: np.ndarray, kappa_samples: np.ndarray, output_path: Path) -> None:
    """
...
def plot_convergence(kappa_samples: np.ndarray, cov_matrix: np.ndarray, 
                    output_path: Path, n_samples: int, nx: int) -> None:
    """
...
def generate_random_field(n_samples: int, lx: float, ly: float, 
                        correlation_length: float, nx: int, ny: int, 
                        output_folder: str, mu_target: float, 
                        sigma_target: float, error_threshold: float) -> None:
    """
    ...
    eig_vals, eig_vecs, n_truncate = compute_kl_decomposition(cov_matrix, error_threshold=error_threshold)
...
def parse_arguments():
    ...
    parser.add_argument('--lc', type=float, default=0.2,
    ...
    parser.add_argument('--mu', type=float, default=17,
                        help='Target mean for non-Gaussian field')
    parser.add_argument('--sigma', type=float, default=0.3,
                        help='Target standard deviation for non-Gaussian field')
    parser.add_argument('--output', type=str, default='randomfield',
                        help='Output folder path')
    parser.add_argument('--error_threshold', type=float, default=1e-3,
...
if __name__ == '__main__':
    ...
    default_nx = 32
    default_ny = 32
    default_mu_target = 17 # stainless steel conductivity of 17 W/mK 
    ...
    # compute correlation length, correlation length must be >= 4 * discretization length
    lc = 8 * lx / nx
    
    # Generate random field
    generate_random_field(
        n_samples, lx, ly, lc, nx, ny, output_folder, mu_target, sigma_target, error_threshold)